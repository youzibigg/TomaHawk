...
are **no package dependencies**, so no install step is required. The optional
refresh command regenerates the bundled Natural Earth coastline asset before
starting the server.
要求：支持 ES 模块的现代 Node.js 与桌面浏览器；无软件包依赖，无需安装步骤。可选刷新命令会在
